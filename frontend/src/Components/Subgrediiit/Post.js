...
const Post = ({ id, text, postedBy, postedIn, numUpvotes, numDownvotes, comments, onUpvote, onDownvote, onReport, postComment}) => {
  ...
  const followHandler = async () => {
    try {
      ...
      const response = await axios.post(`http://localhost:3500/follower`, data, { headers });
...
const handleSave = async () => {
  ...
  const pathname = window.location.pathname
    ...
    try {
        ...
        const response = await axios.post(`http://localhost:3500/subgrediiit/${last}/save`, data, { headers });
      ...
      <CardContent>
        ...
        <Box display="flex" justifyContent="space-between" alignItems="center" mt={2}>
          ...
          <Box display="flex" justifyContent="flex-end">
            <Button  variant="outlined" color="secondary" size="small" sx={{ fontSize: '0.8rem', padding: '0.2rem 0.5rem' }} onClick={followHandler} >
              Follow
            </Button>
          </Box>
        </Box>
        ...
        {comments && (
          ...
          <Box display="flex" flexDirection="column" alignItems="flex-start" mt={2}>
            ...
            <Button variant="outlined" color="primary" size="small" sx={{ fontSize: '0.8rem', padding: '0.2rem 0.5rem', mt: '0.5rem' }} onClick={handleSave}>
              Save
            </Button>
          </Box>
        </Box>
